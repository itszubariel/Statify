...
export function activate(context: vscode.ExtensionContext) {
    ...
    const dashboardCmd = vscode.commands.registerCommand('project-stats.openDashboard', async () => {
        ...
        const stats = await gatherStats(folders[0].uri.fsPath);
        
        // Track project growth history
        const historyKey = `projectGrowth_${folders[0].uri.fsPath}`;
        let growthHistory: Array<{date: string; lines: number; files: number}> = context.workspaceState.get(historyKey) || [];
        ...
        if (!lastEntry || lastEntry.date !== today) {
            ...
            if (growthHistory.length > 30) {
                growthHistory = growthHistory.slice(-30);
            }
            
            await context.workspaceState.update(historyKey, growthHistory);
        }
        
        // Calculate growth since last scan (today-first or previous day)
        let previousEntry = growthHistory.length > 1 ? growthHistory[growthHistory.length - 2] : null;
        if (lastEntry && lastEntry.date === today) {
            // Same day: use first entry of today for delta
            const todayFirstIndex = growthHistory.findIndex(entry => entry.date === today);
            if (todayFirstIndex > 0) {
                previousEntry = growthHistory[todayFirstIndex];
            }
        }
        const growth = {
            linesDelta: previousEntry ? stats.codeStats.totalLines - previousEntry.lines : 0,
            filesDelta: previousEntry ? stats.totalFiles - previousEntry.files : 0,
            history: growthHistory
        };


        panel.webview.html = getWebviewContent(stats, panel.webview, folders[0].uri.fsPath, growth);
        ...
        panel.webview.onDidReceiveMessage(async msg => {
            if (msg.command === 'openFile') {
                ...
                const newStats = await gatherStats(folders[0].uri.fsPath);
                
                // Update growth history on refresh
                const newToday = new Date().toISOString().split('T')[0];
                let newHistory: Array<{date: string; lines: number; files: number}> = context.workspaceState.get(historyKey) || [];
                const newLastEntry = newHistory[newHistory.length - 1];
                
                if (!newLastEntry || newLastEntry.date !== newToday) {
                    newHistory.push({
                        date: newToday,
                    ...
                    await context.workspaceState.update(historyKey, newHistory);
                } else {
                    // Update today's entry
                    newLastEntry.lines = newStats.codeStats.totalLines;
                    newLastEntry.files = newStats.totalFiles;
                    await context.workspaceState.update(historyKey, newHistory);
                }
                
                // Same day logic for refresh
                let newPrevious = newHistory.length > 1 ? newHistory[newHistory.length - 2] : null;
                const newTodayEntry = newHistory.find((entry, index) => entry.date === newToday && index === newHistory.findIndex(e => e.date === newToday));
                if (newTodayEntry && newHistory.length > newHistory.findIndex(e => e.date === newToday) + 1) {
                    newPrevious = newHistory[newHistory.findIndex(e => e.date === newToday)];
                }
                const newGrowth = {
                ...
                panel.webview.html = getWebviewContent(newStats, panel.webview, folders[0].uri.fsPath, newGrowth);
            }
        });
    });
...
async function gatherStats(root: string) {
    ...
    // Define extensions
    const codeExts = ['ts','js','jsx','tsx','py','java','c','cpp','cs','html','css','scss','json','md'];
    const mediaExts = ['png','jpg','jpeg','gif','mp4','mov','avi','webm','mp3','wav','ogg'];
...
const mediaStats = { totalFiles:0, totalSize:0, biggest: null as FileItem | null, files: [] as {path: string, size: number}[], topFiles: [] as {path: string, size: number}[] };
    ...
    // Track most recently edited files (by modification time)
    ...
    for (const file of files) {
        ...
        if (codeExts.includes(ext)) {
            let content = '';
            try { content = fs.readFileSync(file.fsPath, 'utf-8'); } catch {}
            ...
            // Track recent edits (within last 30 days)
    ...
    // Git detection and info
    let gitInfo: any = { isRepo: false };
    ...
    try {
        ...
        // Get commit activity for heatmap (last year)
    ...
    // File modification tracking (using file system modification times)
    ...
    try {
        ...
        // Only look at files modified in the last year
...
const codeTopFiles: FileItem[] = files.filter(f => codeExts.includes(path.extname(f.fsPath).replace('.', '').toLowerCase()))
        .map(f => ({ path: vscode.workspace.asRelativePath(f.fsPath), size: fs.statSync(f.fsPath).size }))
        .sort((a,b)=>b.size-a.size).slice(0,7);
...
function getWebviewContent(stats: any, webview: vscode.Webview, root: string, growth: any) {
    ...
    // Calculate language percentages and sort by lines
    ...
    const langColors: Record<string, string> = {
        ...
        'json': '#292929', 'md': '#083fa1', 'other': '#858585'
    };

    // Create GitHub-style language bars
    const langBarsHTML = langStats.map(l => {
        const color = langColors[l.lang] || '#858585';
        return `
        <div class="lang-item">
            <div class="lang-info">
    ...
    // Dual activity heatmap (saves + commits - last 12 weeks)
    const weeksToShow = 12;
    const saveHeatmapHTML = generateHeatmap(dailySaves, weeksToShow);
    const commitHeatmapHTML = gitInfo.isRepo ? generateHeatmap(commitActivityData, weeksToShow) : '<div class="empty-state" style="padding: 20px; text-align: center;">No git history</div>';
...
<title>Project Stats Dashboard</title>
<style>
...
<div class="grid">
    ...
    <div class="card wide-card">
        <div class="card-title">
            ...
            Save Activity (Last ${weeksToShow} Weeks)
        </div>
${dualHeatmapHTML}
    </div>
    ...
    <div class="card">
        ...
        <div class="stat-grid">
            ...
            <div class="stat-item">
                ...
                <div class="stat-value" style="font-size: 1rem; color: #8b949e;">${growth.history.length} days</div>
            </div>
        </div>
...
function generateHeatmap(dailySaves: Array<{date: string; count: number}>, weeks: number): string {
    ...
    for (let w = 0; w < weeks; w++) {
        ...
        for (let d = 0; d < 7; d++) {
            ...
            html += `<div class="heatmap-day level-${level}" title="${dateStr}: ${count} saves"></div>`;
        }
        html += '</div>';
    }
